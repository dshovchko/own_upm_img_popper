...
#no{margin-left:2em;}
#return,#success,#failed{padding:1em;text-align:center;background-color:#fff;border:5px solid #fc3;}
-->
</style>
...
css;
		...
		if ($rs)
		{
			...
			while ($a = nextRow($rs))
			{
				...
				$ids = n.'<ul>'.n.
					n.t.'<li>'.$id.'</li>'.
					n.t.'<li><a href="'.$edit_url.'" onclick="warnEditImage('.$id.'); return false;">'.gTxt('edit').'</a></li>'.
					n.t.'<li>'.safe_strftime('%d %b %Y %I:%M %p', $uDate).'</li>'.
					n.'</ul>';
			...
			echo endTable().
				...
				nav_form_upm('upm_img_popper','1', $page, $numPages, $sort, $dir, $crit, $search_method).
...
// ---
	...
	function nav_form_upm($event, $bm, $page, $numPages, $sort, $dir, $crit, $search_method, $total=0, $limit=0, $step='list')
	{
		global $theme;
		if ($crit != '' && $total > 1)
		{
